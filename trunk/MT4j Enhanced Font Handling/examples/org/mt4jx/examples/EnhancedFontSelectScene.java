...
import org.mt4jx.components.visibleComponents.font.EnhancedFontManager;

public class EnhancedFontSelectScene extends AbstractScene {

  private static final String DEMO_TEXT = "This version of the test program uses the EnhancedFontManager";

  private MTTextArea demonstrationText;
  private Timer textUpdateTimer;
  
  public EnhancedFontSelectScene(MTApplication mtApplication, String name) {
    ...
    EnhancedFontManager fm = EnhancedFontManager.getInstance();
    
    IFont font = fm.createFontByName(mtApplication, "Arial", 35, MTColor.WHITE);
  ...
  private void chooseFont() {
    ...
    IFont font = EnhancedFontManager.getInstance().createFont(this.getMTApplication(),
  ...
  private void setDemonstrationTextFont(String fontName) {
    ...
    try {
      ...
      IFont newFont = null;
      boolean usingDefaultFont = false;
      
      if (fm.isFontAvailable(fontName)) {
        newFont = fm.createFont(this.getMTApplication(), 
          fontName, oldFont.getOriginalFontSize(), oldFont.getFillColor());
      } else {
        newFont = fm.createFont(this.getMTApplication(), 
            fm.defaultFontName(), oldFont.getOriginalFontSize(), oldFont.getFillColor());
      ...
      if (newFont != null) {
        ...
        if (usingDefaultFont) {
          demonstrationText.setText(fontName + " was not available. Using default font " + fm.defaultFontName());
        } else {
          
          String filename = fm.fontFilePath(fontName);
